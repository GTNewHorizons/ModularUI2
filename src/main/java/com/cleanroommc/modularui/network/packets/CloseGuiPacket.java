...
public class CloseGuiPacket implements IPacket {
    ...
    public @Nullable IPacket executeClient(NetHandlerPlayClient handler) {
        ModularNetwork.CLIENT.closeContainer(this.networkId, this.dispose, Minecraft.getMinecraft().thePlayer, false);
        return null;
    }

    @Override
    public @Nullable IPacket executeServer(NetHandlerPlayServer handler) {
        ModularNetwork.SERVER.closeContainer(this.networkId, this.dispose, handler.playerEntity, false);
        return null;
    }
}
